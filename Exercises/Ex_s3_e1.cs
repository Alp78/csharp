/*
Series 3 Exercise 1: Deign a stack
...
namespace ExerciceStack {
    ...
    public class Stack {
        ...
        public void Push(object obj){
            ...
            if (obj == null) {
                throw new System.InvalidOperationException("Object cannot be null.");
            } else{
                this.objList.Add(obj);
                Console.WriteLine("Object {0} added on top of the list. Type: {1}", obj, obj.GetType());
            }

        }
        
        public object Pop(){
            lastIndex = this.objList.Count - 1;
            var removedObj = new Object();
            if (lastIndex < 0) {
                throw new System.InvalidOperationException("The list is empty.");
            } else{
                Console.WriteLine("Object {0} was removed from stack. Type: {1}", this.objList[lastIndex], this.objList[lastIndex].GetType());                
                removedObj = this.objList[lastIndex];
                this.objList.RemoveAt(lastIndex);
                return removedObj;
        ...
        public void Clear(){
            ...
            if (lastIndex < 0) {
                ...
                this.objList.RemoveRange(0, this.objList.Count);
    ...
    class Program {
        static void Main (string[] args) {
            ...
            for (int i = 0; i < stack.objList.Count; i++)
            {
                Console.WriteLine(stack.objList[i]);
            }

            // removing the last in with Pop()
            stack.Pop();
            
            // recounting for demonstration
            for (int i = 0; i < stack.objList.Count; i++)
            {
                Console.WriteLine(stack.objList[i]);
            }

            // removing all items from the list with Clear()
            stack.Clear();
